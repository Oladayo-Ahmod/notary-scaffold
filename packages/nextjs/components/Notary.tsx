...
const Notary = ({ hash }: any) => {
  ...
  const { data: rawNotary }: any = useScaffoldReadContract({
    contractName: "Notary",
  ...
  const { writeContractAsync: notarize } = useScaffoldWriteContract("Notary");
    ...
    <div className={"shadow-lg relative rounded-b-lg"}>
      <p className="group">
        <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden bg-white xl:aspect-w-7 xl:aspect-h-8 ">
          ...
          <span className={"absolute z-10 right-0 mt-4 bg-amber-400 text-black p-1 rounded-l-lg px-4"}>
            {/* {product.sold} sold */}
          </span>
